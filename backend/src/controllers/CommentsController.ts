...
import { getComments as repoGetComments, createComment as repoCreateComment, deleteComment as repoDeleteComment, updateComment as repoUpdateComment } from '../repositories/CommentsRepository';
...
export const deleteComment = async (req: Request, res: Response) => {
  try {
    ...
    if (!result) return res.status(403).json({ error: 'Sadece kendi yorumunuzu silebilirsiniz.' });
...
export const updateComment = async (req: Request, res: Response) => {
  try {
    ...
    if (!updated) return res.status(403).json({ error: 'Sadece kendi yorumunuzu güncelleyebilirsiniz.' });
